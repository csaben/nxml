...
Every model revision exposes immutable `artifact_id = sha256:<checkpoint digest>` and an authenticated relative `artifact_uri`. Edge downloads that URI from the control-plane base URL; it never interprets `checkpoint_path`. The server accepts only canonical regular files owned by its service UID beneath the configured checkpoint root, recomputes SHA-256 before streaming, and returns `Content-Length`, digest-backed `ETag`, `Digest`, and `X-Checksum-SHA256` headers. Missing, symlinked, outside-root, identity-mismatched, or corrupt artifacts fail closed.
