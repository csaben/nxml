...
from nxwm_mira.codec.dino import DinoModel, DinoPerceptualLoss
...
def _build_lpips() -> nn.Module:
    """Build a VGG LPIPS module mapping ``(pred, target)`` in ``[-1, 1]`` to a loss.

    Prefers the reference pip ``lpips`` package (what MIRA trained with); falls back to
    torchmetrics' implementation when ``lpips`` is unavailable (it is unmaintained and may
    not install on newer Pythons). Both are differentiable w.r.t. ``pred`` and expect
    ``(N, 3, H, W)`` in ``[-1, 1]``.
    """
    try:
        import lpips
        ...
        return LearnedPerceptualImagePatchSimilarity(net_type="vgg").eval()
...
class CodecLoss(nn.Module):
    ...
    def __init__(self, weights: CodecLossWeights):
        ...
        if self.weights.loss_lpips_perceptual > 0:
            self.lpips_perceptual_loss = _build_lpips()
