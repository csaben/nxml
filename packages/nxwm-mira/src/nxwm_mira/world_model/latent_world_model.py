...
from nxwm_mira.world_model.config import LatentWorldModelConfig, WorldModelInferenceConfig
from nxwm_mira.world_model.diffusion_transformer import DiffusionTransformer
from nxwm_mira.world_model.layers.action_encoder import ActionEncoder
from nxwm_mira.world_model.schedule import build_inference_schedule
...
def _config_dict_from_yaml(raw: dict) -> dict:
    """Resolve a saved architecture config to a plain dict, stripping hydra/removed-field cruft.

    Accepts both this package's layout (``model.config``) and MIRA's (``model.architecture.config``).
    """
    model_section = raw["model"]
    node = (
        model_section["architecture"]["config"]
        if "architecture" in model_section
        else model_section["config"]
    )
    assert isinstance(node, dict), "expected a mapping at the world-model config node"
    return drop_removed_fields(strip_hydra_targets(node), REMOVED_CONFIG_FIELDS)  # type: ignore[return-value]
...
class LatentWorldModel(nn.Module):
    ...
    def __init__(self, config: LatentWorldModelConfig) -> None:
        ...
        self.action_encoder = ActionEncoder(
            action_dim=config.actions.action_dim,
            n_stick_axes=config.actions.n_stick_axes,
    ...
    def denoise_streaming(
        ...
        last_kv_cache = None
        for step_idx, (timestep, delta_t) in enumerate(zip(timesteps[:-1], delta_ts, strict=True)):
    ...
    def visualize(self, outputs: InferenceOutputs) -> dict[str, Tensor]:
        """Render a rollout comparison for W&B logging.

        Marks the predicted (non-context) frames with a coloured border and stacks the
        prediction over the ground truth vertically. (MIRA additionally drew a keyboard HUD;
        a Switch-controller HUD can be added later.)
        ...
        from nxwm_mira.training.wandb_media import (
            add_prediction_border,
            video_to_uint8,
        )

        pred_video = video_to_uint8(outputs.output_video.to("cpu"))
    ...
    def load_from_checkpoint(
        ...
        config = LatentWorldModelConfig.model_validate(_config_dict_from_yaml(config_raw))
        model = cls(config)
        model.to(device)
