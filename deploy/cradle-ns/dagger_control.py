...
class Arbitrator:
    ...
    def apply(
        ...
        if self.mode is Mode.PURE_AI:
            return self._owned(now_ns, policy, "policy", 2, human=human)
        gesture = (
            fresh_h
        ...
        return (
            self._owned(now_ns, human, "human", 1, policy=policy)
            if self._takeover and fresh_h
            else self._owned(now_ns, policy, "policy", 2, human=human)
        )

    def _owned(self, now, proposal, source, owner, *, human=None, policy=None):
